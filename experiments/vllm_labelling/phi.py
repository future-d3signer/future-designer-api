...
class DatasetProcessor:
    ...
    def _create_conversation(self, base64_image: str) -> List[Dict]:
        return [
            {"role": "system", "content": self.SYSTEM_PROMPT},
            {"role": "assistant", "content": "I will analyze the image and respond with a valid JSON object following the exact schema."},
            {"role": "user", "content": [
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}},
                {"type": "text", "text": "Describe this furniture piece in JSON format."}
            ]}
        ]

    def get_json_output(self, output_text):
        start_idx = output_text.find('{')
        end_idx = output_text.rfind('}') + 1
        if start_idx != -1 and end_idx != -1:
            return output_text[start_idx:end_idx]    
        return None
    
    def process_image(self, image: Image.Image) -> FurnitureMetadata:
        try:
            base64_image = self._encode_image(image)
            conversation = self._create_conversation(base64_image)
            outputs = self.llm.chat(conversation, sampling_params=self.sampling_params, use_tqdm=False)
            out = self.get_json_output(outputs[0].outputs[0].text)
        
            if json_data := self._parse_json_response(out):
...
if __name__ == "__main__":
    main(
        ...
        output_repo="filnow/futniture-phi-3.5-vl-4b",
        split_name="test"
    )
